(function(window,document){
  ...
  C.renderCompanions=function(){
    ...
    box.innerHTML=list.length?list.map(function(c){return'<div class="c-card"><div class="yp-card-top"><h3>'+YP.text(c.nickname||c.realName,'陪诊师')+'</h3>'+YP.statusTag('ACCEPTED','已认证')+'</div><p>'+YP.text(c.serviceTypes,'陪诊服务')+' · '+YP.text(c.serviceArea,'服务区域待补充')+' · '+Number(c.experienceYears||0)+' 年经验 · ★ '+Number(c.rating||0).toFixed(1)+'</p><p class="yp-card-note">'+YP.text(c.introduction,'暂未填写个人介绍')+'</p><div class="yp-card-meta"><span>已完成 '+Number(c.completedCount||0)+' 单</span><span>'+YP.text(c.traits,'耐心、细致')+'</span></div><div class="yp-actions"><button class="yp-btn primary small" type="button" data-select-companion="'+YP.attr(c.id)+'">指定该陪诊师</button></div></div>'}).join(''):YP.empty('暂无符合条件的陪诊师');
    box.querySelectorAll('[data-select-companion]').forEach(function(el){el.addEventListener('click',function(){var c=S.companions.find(function(x){return String(x.id)===String(el.dataset.selectCompanion)});S.selectedCompanion=c;S.requestTab='create';C.setActive(C.pages.create,'data-request-tab','create');C.renderRequests();window.goPage(2);YP.toast('已选择 '+(c.nickname||c.realName)+'，请填写需求后提交审核')})});YP.bindInteractive(box);
  };
