...
public interface CompanionProfileMapper {
    ...
    @Select("SELECT id, user_id, real_name, avatar, introduction, service_area, service_types, " +
            "experience_years, rating, completed_count, audit_status, created_at, updated_at " +
            "FROM companion_profile WHERE user_id = #{userId}")
    CompanionProfile selectByUserId(@Param("userId") Long userId);

    /** 审核通过的陪诊师列表，支持按区域/类型筛选，按评分和完成数排序 */
    @Select("<script>" +
            "SELECT cp.id, cp.user_id, cp.real_name, cp.avatar, cp.introduction, cp.service_area, " +
            "cp.service_types, cp.experience_years, cp.rating, cp.completed_count, cp.audit_status, " +
    ...
    @Select("SELECT id, user_id, real_name, avatar, introduction, service_area, service_types, " +
            "experience_years, rating, completed_count, audit_status, created_at, updated_at " +
            "FROM companion_profile WHERE audit_status = 0 ORDER BY created_at DESC")
    ...
    @Insert("INSERT INTO companion_profile(user_id, real_name, avatar, introduction, service_area, " +
            "service_types, experience_years, audit_status, created_at, updated_at) " +
            "VALUES(#{userId}, #{realName}, #{avatar}, #{introduction}, #{serviceArea}, " +
            "#{serviceTypes}, #{experienceYears}, 0, NOW(), NOW())")
    ...
    @Update("UPDATE companion_profile SET real_name = #{realName}, avatar = #{avatar}, " +
            ...
            "experience_years = #{experienceYears}, audit_status = 0, updated_at = NOW() " +
            "WHERE id = #{id}")
    int update(@Param("id") Long id, @Param("realName") String realName, @Param("avatar") String avatar,
               ...
               @Param("serviceTypes") String serviceTypes, @Param("experienceYears") Integer experienceYears);
